"""
generate_urbanvideo.py — UrbanVideo-Bench evaluation with the Robust-TO pipeline.

UrbanVideo-Bench (paper Section B.3) is outdoor embodied spatial reasoning over
urban driving, with four multiple-choice tasks:
    LP = Landmark Position, CF = Counterfactual,
    PE = Progress Evaluation, AG = Action Generation.
Metric: accuracy (%).  Optionally evaluate the RoVA-V1-corrupted variants
(Motion Blur / Gaussian Noise / Glare / Occlusion / Low-Light) via --corruption.
...
The expected metadata JSON is a list of records, each like:
    {
      ...
      "options": ["A. ...", "B. ...", "C. ...", "D. ..."],
      "answer": "B",
      "task": "LP"            # one of LP / CF / PE / AG
    }
Adjust load_records() to match your local UrbanVideo-Bench release if its schema
differs; everything else is benchmark-agnostic.

Usage:
    python evals/generate_urbanvideo.py \
        --data_path /path/to/UrbanVideo-Bench \
        --json_meta evals/urbanvideo_meta.json \
...
def parse_args():
    ...
    p.add_argument("--data_path",  default="/path/to/UrbanVideo-Bench")
    p.add_argument("--json_meta",  default="evals/urbanvideo_meta.json")
    p.add_argument("--output_dir", default="results")
    p.add_argument("--corruption", default="clean",
                   choices=["clean", "MB", "GN", "GL", "Occ", "LL"],
                   help="RoVA-V1 corruption variant to evaluate (default: clean)")
...
def main():
    ...
    per_task = defaultdict(lambda: [0, 0])  # task -> [correct, total]
    outputs = []
    for rec in tqdm(records, desc=f"UrbanVideo-Bench [{args.corruption}]"):
        sub = "clean" if args.corruption == "clean" else os.path.join("corrupted", args.corruption)
        video_path = os.path.join(args.data_path, sub, rec["video"])
        try:
    ...
    tag = f"urbanvideo_{args.corruption}"
    with open(os.path.join(args.output_dir, f"{tag}_predictions.json"), "w") as f:
